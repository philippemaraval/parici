(() => {
  ...
  function appendWeeklyDailyPodiumLeaderboard(rootElement, podiumRows) {
    ...
    table.innerHTML = '<thead><tr><th>#</th><th>Joueur</th><th>Podiums</th><th title="Semaines jou\xE9es">Part.</th></tr></thead>';
    const tbody = document.createElement("tbody");
    podiumRows.forEach((row, index) => {
      ...
      const playerAvatar = row.avatar || "\u{1F464}";
      tr.innerHTML = `<td>${index + 1}</td><td><span class="leaderboard-avatar">${playerAvatar}</span>${row.username || "Anonyme"}</td><td class="daily-podium-counts">\u{1F947}${row.first_places || 0} \u{1F948}${row.second_places || 0} \u{1F949}${row.third_places || 0}</td><td>${row.weeks_played || 0}</td>`;
      tbody.appendChild(tr);
    });
    table.appendChild(tbody);
